...
export const GradientFillIcon = ({isPicked,onPress}:IconProp) => {
    ...
    return (
        <TouchableOpacity  style={{width:'13%', height:'100%', marginHorizontal:'10%', opacity:isPicked?1:0.3}} onPress={handlePress}>
            <Svg onLayout={p => onZoneLayout(p.nativeEvent.layout)}>
                ...
                L ${x1} ${y + zoneWidht}
                A ${12} ${12} 0 0 1 ${x1} ${y - zoneWidht}
                L ${x1} ${y - zoneWidht} 
                `} fill={"url(#grad)"} />
                <Circle cx={thumb0x} cy={y} r={thumbR} fill={'white'} />
                <Circle cx={thumb1x} cy={y} r={thumbR+1} fill={'none'} stroke={'white'} strokeWidth={2}/>
            </Svg>
        </TouchableOpacity >
    )
  }